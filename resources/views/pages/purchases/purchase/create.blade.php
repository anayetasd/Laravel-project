...
<?php
   ...
   $purchases_last=Purchase::max('id');
?>
...
<style>
  ...
  textarea{width: 60%;}
        ...
        <div class="invoice p-3 mb-3">
          <h2 class="text-center mb-3">Purchases Order</h2>
          <div class="row">
            <div class="col-12">
              <h4>
            ...
            <div class="col-sm-4 invoice-col">
              ...
              <table>
                <tr>
                  ...
                  <td><input type="text" id="purchase_id" name="purchase_id" value="{{$purchases_last+1}}" style="width:60px" readonly /></td>
                </tr>
                <tr>
...
function refreshTable() {
  ...
  cart.forEach((item, i) => {
    let line = (item.qty * item.price) - item.discount;
    subtotal += line;
  ...
  document.getElementById("net-total").innerText = subtotal;
}
...
document.getElementById("btnProcessPurchase").addEventListener("click", function () {
  ...
  let data = {
    ...
    warehouse_id: document.querySelector("select[name='warehouse']").value,
  ...
  fetch("http://localhost/laravel12/project_app/public/api/purchases", {
    method: "POST",
    headers: {
    ...
    .then(async res => {
      ...
      try {
        ...
        if (json.msg === "Success") {
          alert("Purchase ID " + json.id + " saved.");
          cart = [];
          refreshTable();
        } else {
